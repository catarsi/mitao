    ...
    def cat_topics_barchart(self, input_files, param, tool_id):
        ...
        function draw_chart(obj_data, x_data, x_att, chart_type, data_type) {
          ...
          for (var cat in obj_data) {
            ...
            var trace_color = hexToRgbA(color,opacity);
            if(cat == "others"){
              trace_color = "#CCCCCC";
            }
        ...
        function check(g= null, v=null) {
                ...
                for (var i = 0; i < app_x_data.length; i++) {
                  ...
                  index_top_topics_counts[app_x_data[i]] = new Array();
                ...
                for (var k_cat in filtered_data) {
                      ...
                      for (var i = 0; i < app_x_data.length; i++) {
                        ...
                        if (index_top_topics_counts[app_x_data[i]].length < top_topics) {
                          index_top_topics[app_x_data[i]].push(k_cat)
                          index_top_topics_counts[app_x_data[i]].push(val_x_counts)
                        }else {
                          var k_cat_others_key = k_cat;
                          var k_cat_others_val = val_x_counts;
                          for (var n = 0; n < index_top_topics_counts[app_x_data[i]].length; n++) {
                            if (val_x_counts > index_top_topics_counts[app_x_data[i]][n]){
                              /*switch a top value and move the old top value to the others category*/
                              k_cat_others_key = index_top_topics[app_x_data[i]][n];
                              k_cat_others_val = index_top_topics_counts[app_x_data[i]][n];
                              index_top_topics_counts[app_x_data[i]][n] = val_x_counts;
                              index_top_topics[app_x_data[i]][n] = k_cat;
                              break;
                            }
                          }
                          index_other_topics[app_x_data[i]]["categories"].add(k_cat_others_key);
                          index_other_topics[app_x_data[i]]["counts"] += k_cat_others_val;
                        }
                      }
                }
                console.log(index_top_topics,index_top_topics_counts,index_other_topics);
                ...
                for (var k_cat in obj_data) {
                  ...
                  for (var x_key in obj_data[k_cat]) {
                    ...
                    // -> make its count 0
                    if (index_top_topics[x_key].indexOf(k_cat) == -1){
      ...
      <body>
              ...
              for (var m_att in index_meta) {
                ...
                if (l_meta_values.length > 0) {
                  ...
                  filter_box = "<div class='m-box'><button id='"+m_att+"_btn' type='button' class='btn btn-light'>Metadata attribute: "+m_att+"</button><span class='pre' id='"+m_att+"_pre'>+</span><div id='"+m_att+"_values' class='m-box-values'>" + filter_box.substring(0, filter_box.length-4)+"</div>";
                }
